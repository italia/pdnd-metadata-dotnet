...
La libreria nasce per un’esigenza molto pratica: quando esponi un e-service come **provider (erogatore)**, spesso devi capire *chi sta chiamando*, *con quale contesto PDND*, e *come la chiamata può essere correlata e auditata*, senza disseminare parsing “ad hoc” degli header tra controller, minimal API e middleware.
...
- estrae informazioni PDND in modalità best-effort e non bloccante,
- rende raggiungibili “safe defaults” (niente `Authorization` raw, niente blob firmati di default),
- e mantiene separata l’integrazione ASP.NET Core dalla logica core di estrazione.

## Panoramica PDND

**PDND** abilita lo scambio sicuro e tracciabile di servizi/dati tra amministrazioni. Nel pattern di interazione più comune, il fruitore chiama l’erogatore inviando un **voucher** (JWT) in:
...
La libreria cattura anche segnali standard di correlazione/tracing comunemente usati nei moderni servizi HTTP (es. W3C Trace Context). PDND fornisce indicazioni su pratiche di tracing/osservabilità per il monitoraggio dell’interoperabilità. Riferimento ufficiale: manuale tracing. ([developer.pagopa.it](https://developer.pagopa.it/pdnd-interoperabilita/guides/manuale-operativo-tracing))
...
2. **Estrazione PDND-aware (best-effort)**
   - voucher: parsing del payload JWT ed estrazione di claim selezionati
   - tracking evidence: parsing di header/payload token ed estrazione di campi selezionati
   - digest: parsing del valore dell’header in una coppia normalizzata (alg, value)
   - DPoP: parsing di header/payload proof token ed estrazione di campi selezionati

3. **Comportamento fail-soft**
   - header mancanti ignorati
   - errori di parsing “swallowed”
   - token troppo grandi saltati (guard-rail tramite `MaxTokenLength`)

L’output è pensato per essere stabile e facile da integrare con logging, auditing o dashboard di tracing interne senza esporre segreti.
...
> Nota sugli header forwarded: valori come `Forwarded` / `X-Forwarded-For` sono affidabili solo se impostati da un reverse proxy / API gateway trusted. In reti aperte sono controllabili dall’utente e non vanno considerati segnali di identità autorevoli.
...
- `pdnd.voucher.clientId` (se presente)
...
- `pdnd.dpop.htm`, `pdnd.dpop.htu`, `pdnd.dpop.jti`, `pdnd.dpop.iat` (se presenti)
...
- mantieni `CaptureRawTrackingEvidenceHeader = false` e `CaptureRawDpopHeader = false`
